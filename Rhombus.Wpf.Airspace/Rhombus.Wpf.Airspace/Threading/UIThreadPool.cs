...
namespace Rhombus.Wpf.Airspace.Threading {
    public static class UIThreadPool {
        ...
        private static readonly Dictionary<System.Windows.Threading.Dispatcher, Tuple<bool, int>> _poolThreads = new Dictionary<System.Windows.Threading.Dispatcher, Tuple<bool, int>>();
            ...
            set {
                lock (_guard) {
                    ...
                    while (_numThreads < _minThreads) {
                        UIThreadPool.AddNewThreadToPool();
                    }
                }
            }
        }
            ...
            set {
                lock (_guard) {
                    ...
                    while (_numThreads > _maxThreads) {
                        var dispatcher = UIThreadPool.GetLeastReferencedThread(true);
                        UIThreadPool.RemoveThreadFromPool(dispatcher);
                    }
                }
            }
        }
        ...
        public static UIThreadPoolThread AcquireThread() {
            ...
            lock (_guard) {
                ...
                dispatcher = UIThreadPool.GetLeastReferencedThread(true);
                ...
                if ((dispatcher == null || _poolThreads[dispatcher].Item2 > 0) && _numThreads < _maxThreads) {
                    ...
                    dispatcher = UIThreadPool.GetLeastReferencedThread(false);
                    if (dispatcher == null)
                        dispatcher = UIThreadPool.AddNewThreadToPool();
                }
        ...
        /// </remarks>
        internal static void IncrementThread(System.Windows.Threading.Dispatcher dispatcher) {
            lock (_guard) {
                System.Diagnostics.Debug.Assert(_poolThreads.ContainsKey(dispatcher));

                var refCount = _poolThreads[dispatcher].Item2;
        ...
        /// </remarks>
        internal static void DecrementThread(System.Windows.Threading.Dispatcher dispatcher) {
            lock (_guard) {
                System.Diagnostics.Debug.Assert(_poolThreads.ContainsKey(dispatcher));
                System.Diagnostics.Debug.Assert(_poolThreads[dispatcher].Item2 > 0);

                var inPool = _poolThreads[dispatcher].Item1;
                var refCount = _poolThreads[dispatcher].Item2 - 1;
                ...
                if (refCount == 0 && inPool && _numThreads > _minThreads)
                    ...
                    UIThreadPool.DestroyWorkerThread(dispatcher);
            }
        }
        ...
        /// </returns>
        private static System.Windows.Threading.Dispatcher AddNewThreadToPool() {
            System.Diagnostics.Debug.Assert(_numThreads < _maxThreads, "Creating a thread would exceed the maximum allowed!");

            var isRunning = new AutoResetEvent(false);
            var threadId = _threadIdCounter++;
            ...
            var thread = new Thread(UIThreadPool.WorkerThreadMain);
        ...
        /// </remarks>
        private static void RemoveThreadFromPool(System.Windows.Threading.Dispatcher dispatcher) {
            System.Diagnostics.Debug.Assert(_poolThreads.ContainsKey(dispatcher));
            System.Diagnostics.Debug.Assert(_poolThreads[dispatcher].Item1);
        ...
        ///     The dispatcher associated with the worker thread.
        /// </param>
        private static void DestroyWorkerThread(System.Windows.Threading.Dispatcher dispatcher) {
            System.Diagnostics.Debug.Assert(_poolThreads.ContainsKey(dispatcher));
            System.Diagnostics.Debug.Assert(_poolThreads[dispatcher].Item2 == 0);
        ...
        /// </returns>
        private static System.Windows.Threading.Dispatcher GetLeastReferencedThread(bool inPool) {
            KeyValuePair<System.Windows.Threading.Dispatcher, Tuple<bool, int>>? minPair = null;
            foreach (var pair in _poolThreads) {
                if (pair.Value.Item1 == inPool) {
                    ...
                    if (minPair == null || pair.Value.Item2 < minPair.Value.Value.Item2) {
                        minPair = pair;
                    }
                }
            }
